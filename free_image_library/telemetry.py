...
GATEWAY_URLS = [
    "https://free-image-library.builditwithai.xyz/e",
    # .workers.dev fallback: prevents event loss during DNS propagation or
    # edge routing blips (fleet dual-endpoint standard).
    "https://free-image-library-install-telemetry.reachsuren.workers.dev/e",
]
_env_url = os.getenv("FREE_IMAGE_LIBRARY_TELEMETRY_URL")
...
try:
    ...
    MCP_SERVER_VERSION = importlib.metadata.version("free-image-library")
except Exception:
    MCP_SERVER_VERSION = "unknown"


# Any disable flag wins over FREE_IMAGE_LIBRARY_TELEMETRY=true.
def _telemetry_disabled() -> bool:
    if os.getenv("FREE_IMAGE_LIBRARY_TELEMETRY", "true").lower() in ("false", "0", "off"):
        return True
...
INTERNAL_RUN = os.getenv("FREE_IMAGE_LIBRARY_INTERNAL", "").lower() in ("1", "true", "yes")


def _init_anonymous_identity():
    """Random installation UUID in ~/.free_image_library/; created on first run, reset
    ...
    try:
        config_dir = Path.home() / ".free_image_library"
...
# FREE_IMAGE_LIBRARY_SOURCE, set in install snippets; raw value + low-cardinality bucket.
_KNOWN_SOURCES = {
    "readme", "glama", "mcpso", "pulsemcp", "openimages", "setup",
    "cursor_button", "vscode_button", "installer",
}


def _install_source():
    raw = (os.getenv("FREE_IMAGE_LIBRARY_SOURCE") or "").strip().lower()
    if not raw:
        # curl|bash installer writes ~/.free_image_library/source (env can't survive
        # agent launches); fall back to it so server events carry the bucket.
        try:
            source_file = Path.home() / ".free_image_library" / "source"
    ...
    def _send():
        try:
            props = {
                ...
                "mcp_server_name": "free-image-library",
                "$os": platform.system(),
                ...
                try:
                    req = urllib.request.Request(
                        ...
                        headers={
                            ...
                            # the edge
                            "User-Agent": f"free-image-library/{MCP_SERVER_VERSION}",
                        },
                    )
...
def _track_version_change():
    ...
    try:
        version_file = Path.home() / ".free_image_library" / "last_run_version"
...
def announce_and_fire_boot_events():
    ...
    if IS_FIRST_INSTALL:
        print(
            "free-image-library collects anonymous usage telemetry (no PII, no queries, "
            "no paths — see 'Telemetry & Privacy' in the README). "
            "Opt out any time with FREE_IMAGE_LIBRARY_TELEMETRY=false or DO_NOT_TRACK=1.",
            file=sys.stderr,
        )
